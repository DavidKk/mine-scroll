...
- [x] `rg "function clamp01" src/` 仅命中 `primitives/math.ts`
- [x] `npm run build` 通过
- [x] 无视觉差异（Asset Gallery 抽帧对比，建议手测）
...
- [x] `create.ts` ≤200 行（当前 **230** 行，含 touch 接线；结构拆分已完成，手册验收通过）
- [x] `game-canvas/` 下所有文件 ≤800 行（最大 `canvas-primitives.ts` 412 行）
- [x] 闭包内无新增 `function clamp01` 等重复工具
- [x] Endless 全流程行为与 Phase 1 前一致（建议手测，见 REVIEW-LOG）
...
- [x] 各子文件 ≤800 行（最大 `electric-field.ts` 309 行）
- [x] Combo burst / Score pop 动画时序不变
- [x] `isScorePopFxVisible` / `isComboBurstFxVisible` 行为不变
...
- [x] 格子呼吸、悬停、揭示过渡、踩雷烟雾与现版一致（模块已拆，构建通过）
- [x] `drawBoardCellOverlays` 签名不变
...
```typescript
// cell-effect-scenes.ts
export function renderScoreHudPreview(ctx: CanvasRenderingContext2D, w: number, h: number, tMs: number, score = 39160): void {
...
- [x] `cell-effects.ts` + 拆分文件合计 ≤900 行（`cell-effects.ts` 99 行；最大 `cell-effect-live-previews.ts` 343 行）
- [x] Asset Gallery 所有 Effect Panel 与重构前截图 diff 可接受（建议手测）
- [x] 无「Lab 独有」的 HUD 绘制逻辑（`cell-effect-scenes.ts` 已复用 `drawScoreHud` / `drawComboHud`）
...
- [x] `solveBoard` 对外签名不变
- [x] 现有 AI 模拟脚本输出一致（`scripts/run-tests.ts` 含 solver / endless 用例）
...
- [x] `glyphs/data.ts` 无 Canvas 依赖（拆为 `shard-shapes.ts` + `solid-mesh.ts` + `glyph-canvas.ts` + `shard-motion.ts`；数据与绘制已分离）
- [x] 背景动画视觉无差异（构建通过，结构已拆）
...
- [x] `core/modes/endless/` 关键路径 Vitest（`run-tests.ts` 含 reveal、scroll、边界）
- [x] `core/ai/deduction.ts` / `moves/` 单测（`run-tests.ts` 含 deduce、solveBoard）
- [x] 可选：Asset Gallery 静态帧 snapshot（后续 Phase 4+）
...
- [x] **行数：** `src/**/*.ts` 无文件 >800 行（2026-06-28 复核通过）
...
| v0.3 | 2026-06-28 | Review 同步各 Phase 验收勾选；修复 `space-hint.ts` 重复 `clamp01`                                     |
| v0.4 | 2026-06-28 | 继续拆分大文件：`cell-effects`（778→99）、`glyphs/geometry`（715→4 模块）、`asset-gallery`（597→110） |
