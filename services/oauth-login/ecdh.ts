...
/**
 * Client-side ECDH helpers (from vercel-web-scripts / vercel-2fa).
 */

export async function generateECDHKeyPair(): Promise<CryptoKeyPair> {
  return window.crypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'P-256' }, true, ['deriveKey', 'deriveBits'])
}
...
export async function importPrivateKey(privateKeyBase64: string): Promise<CryptoKey> {
  ...
  return window.crypto.subtle.importKey('pkcs8', privateKeyBuffer, { name: 'ECDH', namedCurve: 'P-256' }, false, ['deriveKey', 'deriveBits'])
}
...
export async function deriveSharedKey(privateKey: CryptoKey, serverPublicKeyBase64: string): Promise<CryptoKey> {
  ...
  return window.crypto.subtle.deriveKey({ name: 'ECDH', public: serverPublicKey }, privateKey, { name: 'AES-GCM', length: 256 }, false, ['decrypt'])
}
...
export async function decryptWithSharedKey(encryptedDataBase64: string, sharedKey: CryptoKey): Promise<string> {
  ...
  const decrypted = await window.crypto.subtle.decrypt({ name: 'AES-GCM', iv, tagLength: 128 }, sharedKey, ciphertextWithTag)
