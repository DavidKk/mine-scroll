import { clamp01, lerp, strokeRoundedLoop, type CosmicPalette } from './shared.ts';
...
import {
  ...
  shmupShipSize,
  shmupShotAtBirth,
  type ShmupShipMotion,
} from './shmup-motion.ts';
...
function drawShmupThruster(
  ...
  const flicker =
    0.72 + Math.sin(nowMs * 0.031) * 0.18 + Math.sin(nowMs * 0.047 + 1.2) * 0.1;
...
export function drawShmupShipLayer(
  ctx: CanvasRenderingContext2D,
  shellW: number,
  shellH: number,
  nowMs: number,
...
export function drawShmupBulletsLayer(
  ctx: CanvasRenderingContext2D,
  shellW: number,
  shellH: number,
  nowMs: number,
