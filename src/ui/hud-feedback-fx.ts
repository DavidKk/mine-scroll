...
function getComboRailArcProfile(combo: number): {
  ...
  const tierCeil = tier >= 3 ? COMBO_HUD_TIER_THRESHOLDS[2] + 48 : COMBO_HUD_TIER_THRESHOLDS[tier];
  const progress =
    tierCeil > tierFloor ? clamp01((combo - tierFloor) / (tierCeil - tierFloor)) : 0;
...
function drawLocalizedElectricField(
  ctx: CanvasRenderingContext2D,
  fieldW: number,
  fieldH: number,
  combo: number,
    ...
    if (i % 3 !== 2) {
      ...
      drawElectricBolt(
        ...
        branchMode === undefined
          ? budget === 'normal' && tier >= 1
          : branchMode && i % 2 === 0,
      );
    }

    const angle =
      seededUnit(combo + i * 2.13 + timeSeed * 0.31) * Math.PI * 2 + t * Math.PI * 3.4;
