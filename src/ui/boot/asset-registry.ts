...
export const SCORE_DIGIT_URLS = Array.from(
  { length: 10 },
  (_, digit) => `/assets/candidates/hud-feedback-v3/runtime/score-digits-v1/digit-${digit}.png`,
);
...
function hudIconAssets(): BootAsset[] {
  const entries: BootAsset[] = [
    ...
    ...HUD_ICON_NAMES.map((name) => ({
      ...
      weight: name.startsWith('volume') ? 256 * 256 : SMALL_SPRITE_WEIGHT,
    })),
  ];
  return entries;
}
...
export const TILE_SPRITE_URLS = {
  ...
  numbers: Array.from(
    { length: 8 },
    (_, i) => `${BOARD_V3_TILE_BASE}/num-${i + 1}.png`,
  ),
} as const;
