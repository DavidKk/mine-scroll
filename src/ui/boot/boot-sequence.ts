...
async function runOnce(options: BootSequenceOptions = {}): Promise<BootResult> {
  ...
  const assetRatio = () =>
    totalWeight > 0 ? MANIFEST_PROGRESS + ASSET_PROGRESS * (completedWeight / totalWeight) : MANIFEST_PROGRESS;

  const emitAssets = () => {
    onProgress?.(
      computeBootProgress(assetRatio(), currentLabel, loadedCount, blocking.length, currentGroup),
    );
  };
