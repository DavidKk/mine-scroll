...
export function updateScrollButtonReveal(
  ...
  const heightScale =
    heightElapsed <= 0
      ? lineHeightScale
      : lineHeightScale + easeOutCubic(heightElapsed / SCROLL_BUTTON_REVEAL_HEIGHT_MS) * (1 - lineHeightScale);
  const contentElapsed = elapsed - SCROLL_BUTTON_REVEAL_CONTENT_DELAY_MS;
  const contentAlpha =
    contentElapsed <= 0
      ? 0
      : easeOutCubic(contentElapsed / SCROLL_BUTTON_REVEAL_CONTENT_MS);
  const animating =
    elapsed < SCROLL_BUTTON_REVEAL_CONTENT_DELAY_MS + SCROLL_BUTTON_REVEAL_CONTENT_MS;
