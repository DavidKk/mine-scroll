...
import { getBoardSideRailLayout, createBoardSideRailGradient } from '../../renderer/index.ts';
import { drawHudIcon, drawIconTextButton } from '../../hud-sprites.ts';
import type { GameCanvasFullscreenOptions } from '../types.ts';
import { drawArcadeGlow, drawArcadePanel, drawRuntimePanelV3Fx, drawUiPanelImage, drawUiPanelImageBounds, comboColor, logColor, logIcon, drawBottomEnergyRail } from '../hud/canvas-primitives.ts';
import { panelTransitionProgress } from './panel-transition.ts';
import { drawDifficultyAlertFullscreenFlash, drawDifficultyAlert, drawFullscreenScrollWarning } from './difficulty-alert.ts';
...
export function drawFullscreenOverlay(rt: GameCanvasRuntime, 
  ...
  if (stats?.scoreEvent && stats.scoreEvent.id !== rt.state.lastScoreEventId) {
    ...
    beginScoreCountUp(rt, stats.scoreEvent.scoreAfter ?? stats.score ?? 0,
  ...
  if (rt.state.currentStatus === 'lost') {
    ...
    rt.state.retryRect = { x: retryX, y: retryY, w: retryW, h: retryH };
    ...
    drawRuntimePanelV3Fx(rt, shellCtx,
  ...
  shellCtx.fillStyle = THEME.overlayScrim;
  shellCtx.fillRect(0, 0, shellW, shellH);
  if (!drawUiPanelImage(rt, shellCtx, 'log-panel', x, y, modalW, modalH, 1.02)) {
    drawArcadePanel(rt, shellCtx, x, y, modalW, modalH, 'rgba(59, 130, 246, 0.72)', 'rgba(3, 8, 18, 0.96)');
  }
  drawArcadeGlow(rt, shellCtx, x + 10, y + 10, modalW - 20, modalH - 20, 'rgba(59, 130, 246, 0.46)', 0.7);
  ...
  fillRounded(shellCtx, x + 18, y + 54, modalW - 36, modalH - 74, 8, 'rgba(10, 16, 32, 0.74)');
  strokeRounded(shellCtx, x + 18.5, y + 54.5, modalW - 37, modalH - 75, 8, 'rgba(59, 130, 246, 0.24)', 1);
  ...
  for (const line of logs.slice(-maxLines)) {
    ...
    drawHudIcon(shellCtx, logIcon(rt, line.kind), x + 84, lineY + 1, { size: 12 });
    shellCtx.fillStyle = logColor(rt, line.kind);
