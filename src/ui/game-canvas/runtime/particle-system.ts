...
export function spawnScoreHudParticles(rt: GameCanvasRuntime): void {
  ...
  const fit =
    HUD_FEEDBACK_ASSETS.scorePanelV6.complete && HUD_FEEDBACK_ASSETS.scorePanelV6.naturalWidth > 0
      ...
      : 1;
  const panelW = HUD_FEEDBACK_ASSETS.scorePanelV6.complete
    ? HUD_FEEDBACK_ASSETS.scorePanelV6.naturalWidth * fit
    : maxW;
  const panelH = HUD_FEEDBACK_ASSETS.scorePanelV6.complete
    ? HUD_FEEDBACK_ASSETS.scorePanelV6.naturalHeight * fit
    : maxH;
