...
} from '../renderer/intro-ripple-math.ts';
...
export function getIdleRippleWaveStrength(
  nowMs: number,
  dist: number,
  maxDist: number,
  cycleMs = IDLE_RIPPLE_CYCLE_MS,
  phaseOffset = 0,
): number {
...
function drawIdleCellRippleOverlay(
...
function drawIdleBoardRippleRings(
  ctx: CanvasRenderingContext2D,
...
function drawIdleRippleForViews(
  ctx: CanvasRenderingContext2D,
  layout: LayoutMetrics,
  views: CellView[],
  rows: number,
