...
export interface NavItem {
  id: string;
  label: string;
  group: 'sprites' | 'animations';
  count?: number;
}

export function createTopbar(activeGroup: 'sprites' | 'animations', onGroupChange: (g: 'sprites' | 'animations') => void): HTMLElement {
  ...
  brand.innerHTML = `
    ...
    <div>
      <h1>Asset Lab</h1>
      <p>Tile slices &amp; animation frames · 1:1 preview</p>
    </div>
  `;
  ...
  const groups: Array<{ id: 'sprites' | 'animations'; label: string }> = [
  ...
  for (const group of groups) {
    ...
    tab.addEventListener('click', () => onGroupChange(group.id));
    tabs.append(tab);
  }
...
export function createFpsControl(initialFps: number, onChange: (fps: number) => void): HTMLElement {
  ...
  const apply = (): void => {
    const fps = Math.max(1, Math.min(24, Number(input.value) || initialFps));
