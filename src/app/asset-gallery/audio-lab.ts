...
function createOverviewPanel(ctx: AudioLabContext): HTMLElement {
  ...
  panel.append(
    createPanelHead(
      ...
      `${AUDIO_LAB_ENTRIES.length} one-shots under public/assets/game/audio. Peak-normalized via MASTER × SFX_GAIN in game-audio.ts.`,
    ),
  );
  ...
  busMeta.innerHTML = `
    ...
    <div><dt>Source</dt><dd><code>src/ui/game-audio.ts</code></dd></div>
  `;
...
function createCategoryPanel(
  ...
  title: string,
  description: string,
  filter: (entry: AudioLabEntry) => boolean,
  ctx: AudioLabContext,
): HTMLElement {
...
function createBgmPanel(ctx: AudioLabContext): HTMLElement {
  ...
  panel.append(
    createPanelHead(
      'Idle background music',
      'Loops while session status is idle (start screen and before the first reveal). Stops when playing begins.',
    ),
  );
  ...
  hint.textContent =
    'Lab toggle uses the same GameAudioController as the game. Native player below is for inspecting the raw asset.';
...
export function mountAudioPanels(): {
  ...
  const panels: Record<AudioPanelId, HTMLElement> = {
    ...
    game: createCategoryPanel(
      'game',
      'Game & scroll',
      'Life, chord, heal, and scroll pressure feedback.',
      (e) => e.category === 'game' || e.category === 'scroll',
      ctx,
    ),
    ui: createCategoryPanel(
      'ui',
      'UI',
      'Canvas HUD hover and click feedback.',
      (e) => e.category === 'ui',
      ctx,
    ),
  };
