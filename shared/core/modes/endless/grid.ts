...
function createCellForWorld(worldRow: number, col: number, seed: number, scrollDepth: number): Cell {
...
function recomputeAdjacentInRange(board: Board, startRow: number, endRow: number): void {
  ...
  for (let row = from; row <= to; row += 1) {
    for (let col = 0; col < board.cols; col += 1) {
      ...
      cell.adjacentMines = getLocalNeighbors(board, row, col).filter(({ row: nr, col: nc }) => board.cells[nr]![nc]!.isMine).length
    }
  }
}
...
function createInitialBoard(seed: number): Board {
  ...
  const cells: Cell[][] = Array.from({ length: rows }, (_, row) => Array.from({ length: ENDLESS_COLS }, (__, col) => createCellForWorld(minRow + row, col, seed, 0)))
...
export function prependRow(board: Board, scrollDepth: number): Board {
  ...
  const seed = board.worldSeed!
  const newRow = Array.from({ length: board.cols }, (_, col) => createCellForWorld(newMinRow, col, seed, scrollDepth))
