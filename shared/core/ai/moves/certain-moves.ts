...
export function makeMove(kind: AiMove['kind'], row: number, col: number, confidence: AiMove['confidence'], reason: string): AiMove {
  return { kind, row, col, confidence, reason }
}

export function pickCertainFlags(board: SolverBoard, deduced: Deduction, blocks: AiBlockedSets | undefined, targetRows: Set<number> | null): AiMove | null {
  ...
  return makeMove('flag', pick.row, pick.col, 'certain', board.endless ? (onBottom ? 'Bottom row certain mine' : 'Certain mine') : 'Certain mine')
}

export function pickCertainReveals(board: SolverBoard, deduced: Deduction, blocks: AiBlockedSets | undefined, targetRows: Set<number> | null): AiMove | null {
  const candidates: AiCoord[] = []
  ...
  const onBottom = pick.row === bottomRow(board)
  return makeMove('reveal', pick.row, pick.col, 'certain', board.endless ? (onBottom ? 'Bottom row reveal' : 'Safe reveal') : 'Safe reveal')
}

export function pickCertainChord(board: SolverBoard, deduced: Deduction, blocks: AiBlockedSets | undefined, targetRows: Set<number> | null): AiMove | null {
  let chords = findSafeChords(board, deduced).filter((c) => !isRevealBlocked(blocks, c.row, c.col))
  if (targetRows) chords = chords.filter((c) => targetRows.has(c.row))
  const pick = pickBest(chords, board)
  if (!pick) return null

  return makeMove('chord', pick.row, pick.col, 'certain', board.endless ? 'Bottom row chord' : 'Chord expandable')
}
