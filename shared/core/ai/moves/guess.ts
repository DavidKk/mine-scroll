...
import { type Deduction, key, parseKey, wouldViolateCluesIfFlagged } from '../deduction.ts'
...
export function guessAllowed(endless: boolean, lives: number, prob: number, bottomEmergency: boolean): boolean {
...
export function pickCspMove(board: SolverBoard, deduced: Deduction, lives: number, blocks: AiBlockedSets | undefined, vars: string[], bottomEmergency: boolean): AiMove | null {
  ...
  for (const [k, p] of [...probs.entries()].sort((a, b) => b[1] - a[1])) {
    ...
    return makeMove('flag', row, col, 'certain', bottomEmergency ? 'Bottom row urgent · flag mine' : 'Flag certain mine')
  }
  ...
  for (const [k, p] of [...probs.entries()].sort((a, b) => a[1] - b[1])) {
    ...
    const cell = board.cell(row, col)
    if (!cell || cell.revealed || cell.flagged) continue
    return makeMove('reveal', row, col, 'certain', bottomEmergency ? 'Bottom row urgent reveal' : 'Safe reveal')
  }
  ...
  if (pick.prob <= 1e-9) {
    return makeMove('reveal', pick.row, pick.col, 'certain', bottomEmergency ? 'Bottom row urgent reveal' : 'Safe reveal')
  }
  ...
  return makeMove(
    ...
    pick.col,
    'guess',
    bottomEmergency ? `Bottom row urgent · prob ${pct}%` : board.endless ? `Probabilistic mine guess ${pct}%` : `Guess mine ${pct}%`
  )
}

export function pickFrontierEdge(board: SolverBoard, deduced: Deduction, lives: number, blocks: AiBlockedSets | undefined, bottomEmergency: boolean): AiMove | null {
  ...
  if (deduced.safe.has(key(pick.row, pick.col))) {
    ...
    return makeMove('reveal', pick.row, pick.col, 'certain', board.endless && onBottom ? 'Bottom row reveal' : 'Safe reveal')
  }
...
export function estimateLocalFrontierRisk(board: SolverBoard, deduced: Deduction, row: number, col: number): number | null {
