...
function clueVariables(board: SolverBoard, row: number, col: number, safe: Set<string>, mines: Set<string>): string[] {
  const out: string[] = []
  for (const nb of board.neighbors(row, col)) {
    const c = board.cell(nb.row, nb.col)
...
export function collectConstraints(board: SolverBoard, safe: Set<string>, mines: Set<string>): Constraint[] {
...
export function findContradictoryFlagMove(board: SolverBoard, mines: Set<string>): { row: number; col: number } | null {
  for (let row = 0; row < board.rows; row += 1) {
    for (let col = 0; col < board.cols; col += 1) {
      if (!board.inConstraints(row, col)) continue
      const cell = board.cell(row, col)
...
export function findMisflaggedSafeMove(board: SolverBoard, safe: Set<string>, mines: Set<string>): { row: number; col: number } | null {
  for (const k of safe) {
...
export function isDirectSafe(board: SolverBoard, row: number, col: number, deduced: Deduction): boolean {
  const k = key(row, col)
...
export function isDirectMine(board: SolverBoard, row: number, col: number, deduced: Deduction): boolean {
  const k = key(row, col)
  ...
  for (let r = 0; r < board.rows; r += 1) {
    for (let c = 0; c < board.cols; c += 1) {
      ...
      const hidden = clueVariables(board, r, c, deduced.safe, new Set()).filter((hk) => !deduced.mines.has(hk) || hk === k)
      const need = cell.adjacentMines - trustedFlags(board, r, c, deduced.mines)
