...
import { applyBootWeightMap } from '../src/ui/boot/boot-weights.ts';
import { dedupeBootAssets, collectBootAssets } from '../src/ui/boot/asset-registry.ts';
import { sortBootAssetsForLoad } from '../src/ui/boot/load-priority.ts';
import { computeBootProgress } from '../src/ui/boot/boot-sequence.ts';
import type { BootAsset } from '../src/ui/boot/types.ts';
import {
  configureBootImageFormatForTests,
  resetBootImageFormatForTests,
  resolveRasterUrl,
} from '../src/ui/boot/image-format.ts';
...
export function testApplyBootWeightMapPrefersWebpBytes(): void {
  const assets = [asset('a', { url: '/assets/a.png', weight: 10 })];
  const weighted = applyBootWeightMap(
    assets,
    { '/assets/a.png': 2048, '/assets/a.webp': 512 },
...
export function testSortBootAssetsForLoadPrioritizesHeavyWithinTier(): void {
  const sorted = sortBootAssetsForLoad([
    asset('light', { tier: 2, weight: 10 }),
    asset('heavy', { tier: 2, weight: 900 }),
    asset('tile', { tier: 1, weight: 1 }),
  ]);
  assert.deepEqual(
