...
import {
  classifyPointerEnd,
  computeGestureThresholds,
  detectSwipeDuringMove,
  isDoubleTap,
  useTouchGestureInput,
} from '../src/ui/game-canvas/input/touch-gesture.ts';
import { isDesktopInputProfile, isMobileInputProfile, isDualInputDebugEnabled, useDesktopMouseInput, useTouchPointerInput } from '../src/ui/game-canvas/input/input-profile.ts';
...
export function testDoubleTapWindow(): void {
  const now = 1000;
  assert.equal(
    isDoubleTap({ row: 1, col: 2 }, now + 250, { lastCellKey: '1,2', lastTapAt: now }),
    true,
  );
  assert.equal(
    isDoubleTap({ row: 1, col: 2 }, now + 301, { lastCellKey: '1,2', lastTapAt: now }),
    false,
  );
  assert.equal(
    isDoubleTap({ row: 1, col: 3 }, now + 100, { lastCellKey: '1,2', lastTapAt: now }),
    false,
  );
}
