...
export function drawHudIcon(ctx: CanvasRenderingContext2D, name: HudIconName, x: number, y: number, options: DrawHudIconOptions = {}): boolean {
...
export function drawLivesRow(ctx: CanvasRenderingContext2D, x: number, y: number, lives: LivesDisplay, iconSize = 14, gap = 3): boolean {
...
export function drawChipLabel(ctx: CanvasRenderingContext2D, x: number, y: number, label: string, align: 'left' | 'center' | 'right', chipW: number, icon?: HudIconName): void {
  const iconSize = 11
  const gap = 5
