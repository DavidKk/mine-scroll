...
import { type BoardPointerState, drawBoardCellOverlays, drawIdleBoardRippleFx } from '../cell-fx.ts'
import { cellPixelOrigin, GRID_PADDING, HUD_GAP, HUD_HEIGHT, PANEL_RADIUS, THEME } from '../theme.ts'
import { drawSpriteInCell, getTileSprites } from '../tile-sprites.ts'
import { drawCell, drawCellMarksOverlay, drawHudPill, drawStatusIcon, hudPillWidth } from './cells.ts'
import { getCellIntroRippleAlpha, getCellIntroRippleDist, getMaxCellIntroRippleDist } from './intro-ripple-math.ts'
...
function drawPlayableCells(ctx: CanvasRenderingContext2D, views: CellView[], layout: LayoutMetrics, state: RenderState): void {
...
function drawPlayableMarks(ctx: CanvasRenderingContext2D, views: CellView[], layout: LayoutMetrics, state: RenderState, animateFlags: boolean): void {
  const { gridOriginX, gridOriginY, grid } = layout
  for (const view of views) {
    const { x, y } = cellPixelOrigin(view.row, view.col, gridOriginX, gridOriginY, grid)
...
function drawPlayableAmbientOverlays(ctx: CanvasRenderingContext2D, views: CellView[], layout: LayoutMetrics, state: RenderState): void {
  const { gridOriginX, gridOriginY, grid } = layout
...
function applyPreviewBandMask(ctx: CanvasRenderingContext2D, mask: PreviewBandMask, originX = 0, originY = 0): void {
...
function drawMaskedPreviewLayer(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, previewRows: number, drawLayer: (layerCtx: CanvasRenderingContext2D) => void): void {
...
function drawIntroCellSurface(ctx: CanvasRenderingContext2D, x: number, y: number, view: CellView, grid: LayoutMetrics['grid'], status: RenderState['status']): void {
...
export function renderBoardDynamicFrame(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, state: RenderState): void {
  const previewRows = state.previewRows ?? 0
  const playableViews = previewRows > 0 ? state.views.filter((view) => !view.preview) : state.views
...
export function renderFrame(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, state: RenderState): void {
  const { width, height, gridOriginX, gridOriginY, resetButton, gridWidth, gridHeight, grid } = layout
  ...
  const hudLeft = state.hudLeftDisplay ?? String(state.mineTotal - state.flagCount).padStart(3, '0')
  const leftPillW = hudPillWidth(hudLeft, state.hudLeftDisplay?.includes('♥') ? 72 : 48, state.hudLeftDisplay?.includes('♥') ? 80 : 88)
  drawHudPill(ctx, 12, pillY, leftPillW, pillH, hudLeft, 'left')

  const hudRight = state.hudRightDisplay ?? String(state.elapsedSeconds).padStart(3, '0')
  const rightPillW = hudPillWidth(hudRight, 48, state.hudRightDisplay?.startsWith('↑') ? 80 : 72)
  drawHudPill(ctx, width - rightPillW - 12, pillY, rightPillW, pillH, hudRight, 'right')

  fillRoundRect(ctx, resetButton.x, resetButton.y, resetButton.size, resetButton.size, resetButton.size / 2, THEME.resetBg)
  strokeRoundRect(ctx, resetButton.x + 0.5, resetButton.y + 0.5, resetButton.size - 1, resetButton.size - 1, resetButton.size / 2, THEME.panelBorder)
  drawStatusIcon(ctx, resetButton.x + resetButton.size / 2, resetButton.y + resetButton.size / 2, state.status)
...
export function getBoardSideRailLayout(layout: LayoutMetrics, previewRows: number): BoardSideRailLayout {
...
export function getBoardSideRailBounds(layout: LayoutMetrics, previewRows: number): { top: number; bottom: number } {
...
function drawBoardSideRails(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, previewRows: number, peakColor = 'rgba(96, 165, 250, 0.18)'): void {
  const { gridWidth } = layout
  const rails = getBoardSideRailLayout(layout, previewRows)
  ctx.save()
  ctx.strokeStyle = createBoardSideRailGradient(ctx, rails.top, rails.bottom, peakColor, rails.fadeInStart, rails.fadeInEnd)
  ctx.lineWidth = 1.5
  ctx.beginPath()
...
export function renderBoardOnlyFrame(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, state: RenderState): void {
