...
import { drawImageContained, getGameUiPanel } from '../game-assets.ts'
import { cellPixelOrigin, GRID_PADDING, HUD_GAP, HUD_HEIGHT, THEME } from '../theme.ts'
...
export function drawScrollPressureBar(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, pressure: ScrollPressureState): void {
  ...
  if (fillW >= 1) {
    fillRoundRect(ctx, barX, barY, fillW, barH, 3, pressure.urgent ? THEME.danger : THEME.warning)
  }

  const countdownName: GameUiPanelName = pressure.urgent ? 'countdown-red' : pressure.progress > 0.62 ? 'countdown-orange' : 'countdown-yellow'
...
export function drawScrollDangerBand(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, pressure: ScrollPressureState, visibleRows: number): void {
  ...
  fillRoundRect(ctx, gridOriginX - 2, rowY - 2, bandW, rowH + 4, grid.cellRadius + 2, gradient)
  ...
  if (boundaryW >= 1) {
    ...
    fillRoundRect(ctx, boundaryX, boundaryY, boundaryW, Math.max(3, grid.cellSize * 0.14), 4, boundaryColor)
    ctx.shadowBlur = 0
  }
  ...
  if (pressure.urgent) {
    strokeRoundRect(ctx, gridOriginX - 2, rowY - 2, bandW, rowH + 4, grid.cellRadius + 2, `rgba(254, 202, 202, ${0.55 + pulse * 0.35})`, 2)
  }

  ctx.restore()
}

export function drawAiHint(ctx: CanvasRenderingContext2D, layout: LayoutMetrics, hint: AiHintDisplay): void {
  ...
  strokeRoundRect(ctx, x - 2, y - 2, grid.cellSize + 4, grid.cellSize + 4, grid.cellRadius + 2, isGuess ? THEME.warning : THEME.accent, 2)
