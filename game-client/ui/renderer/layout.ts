import { computeGridMetrics, getGridOrigin, GRID_PADDING, type GridMetrics, HUD_GAP, HUD_HEIGHT } from '../theme.ts'
...
export function getLayoutMetrics(rows: number, cols: number, maxGrid?: { width: number; height: number }, fixedCellSize?: number): LayoutMetrics {
...
export function getBoardOnlyLayoutMetrics(rows: number, cols: number, maxGrid?: { width: number; height: number }, fixedCellSize?: number): LayoutMetrics {
  const grid = computeGridMetrics(rows, cols, maxGrid, fixedCellSize)
  const gridWidth = cols * grid.cellStep - grid.cellGap + GRID_PADDING * 2
  const gridHeight = rows * grid.cellStep - grid.cellGap + GRID_PADDING * 2
...
export function applyBoardPreviewBand(layout: LayoutMetrics, previewRows: number): LayoutMetrics {
