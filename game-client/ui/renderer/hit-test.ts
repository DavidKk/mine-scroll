...
export function hitTestCell(layout: LayoutMetrics, rows: number, cols: number, x: number, y: number): { row: number; col: number } | null {
  return hitTestCellWithPreview(layout, rows, cols, 0, x, y)
}

export function hitTestCellWithPreview(layout: LayoutMetrics, rows: number, cols: number, previewRows: number, x: number, y: number): { row: number; col: number } | null {
...
export function getCanvasPointerCoords(canvas: HTMLCanvasElement, event: MouseEvent | PointerEvent, logicalSize?: { width: number; height: number }): { x: number; y: number } {
