...
export function difficultyAlertProgress(_rt: GameCanvasRuntime, startedAt: number): { t: number; alpha: number; impact: number; strobe: number; pulse: number } | null {
...
export function resolveDifficultyAlertOrigin(rt: GameCanvasRuntime, shellW: number, shellH: number): { ox: number; oy: number; reach: number } {
  ...
  const reach = Math.hypot(Math.max(ox, shellW - ox), Math.max(oy, shellH - oy)) * 1.12
  return { ox, oy, reach }
}

export function drawDifficultyAlertFullscreenFlash(rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, shellW: number, shellH: number): void {
  ...
  for (let i = 0; i < rayCount; i += 1) {
    const baseAngle = isDanger ? -Math.PI / 2 + (i / rayCount - 0.5) * Math.PI * 1.35 : (i / rayCount) * Math.PI * 2
  ...
  for (let i = 0; i < moteCount; i += 1) {
    const angle = (((i * 9973 + (isDanger ? 90 : 0)) % 360) / 360) * Math.PI * 2 + Math.sin(i * 1.7) * 0.18
...
export function drawFullscreenScrollWarning(
  ...
  const alpha = pressure.urgent ? 0.12 + urgentPulse * 0.1 : Math.max(0, pressure.progress - 0.55) * 0.12
  if (alpha > 0) {
    shellCtx.save()
