...
export function cellAtCoords(rt: GameCanvasRuntime, x: number, y: number): { row: number; col: number } | null {
  ...
  const hit = hitTestCellWithPreview(rt.state.squareLayout!, rt.state.currentRows, rt.state.currentCols, rt.state.currentPreviewRows, x, y)
  if (!hit) return null
  return hit
}

function canvasCoords(rt: GameCanvasRuntime, event: MouseEvent | PointerEvent): { x: number; y: number } {
...
function setBoardPointerLocked(rt: GameCanvasRuntime, cell: { row: number; col: number }, pressed: boolean): void {
  if (rt.state.currentStatus !== 'playing') {
    rt.state.boardPointer = null
    return
  }
...
function reportRankedPointer(rt: GameCanvasRuntime, x: number, y: number): void {
  rt.fullscreen?.rankedInput?.onLayout()
  rt.fullscreen?.rankedInput?.onMove(x, y)
}
...
function handleUiPointerDown(rt: GameCanvasRuntime, x: number, y: number): boolean {
  ...
  if (rt.fullscreen && rt.state.leaderboardRect && insideRect(rt.state.leaderboardRect, x, y)) {
    rt.fullscreen.onUiClick?.()
    rt.fullscreen.onOpenLeaderboard?.()
    rt.scheduleAnimationFrame()
    return true
  }
  ...
  if (rt.fullscreen && rt.state.currentStatus === 'idle' && rt.state.startRect && (rt.fullscreen.showStartOverlay?.() ?? true) && insideRect(rt.state.startRect, x, y)) {
    rt.fullscreen.onUiClick?.()
    beginPanelTransition(rt, 'start', () => rt.fullscreen?.onStart?.())
    return true
  }

  if (rt.fullscreen && rt.state.currentStatus === 'lost' && rt.state.retryRect && insideRect(rt.state.retryRect, x, y)) {
    rt.fullscreen.onUiClick?.()
...
export function onMouseDown(rt: GameCanvasRuntime, event: MouseEvent): void {
  ...
  rt.fullscreen?.rankedInput?.onLayout()
  rt.fullscreen?.rankedInput?.onDown(event.button === 2 ? 2 : 0, x, y, event.buttons)

  if (rt.fullscreen?.isLogOpen?.()) return
  if (rt.fullscreen?.isLeaderboardOpen?.()) return
...
export function onContextMenu(rt: GameCanvasRuntime, event: MouseEvent): void {
  ...
  const { x, y } = canvasCoords(rt, event)
  rt.fullscreen?.rankedInput?.onLayout()
  rt.fullscreen?.rankedInput?.onContextMenu(x, y)
  if (isBothButtons(event)) {
...
export function onDoubleClick(rt: GameCanvasRuntime, event: MouseEvent): void {
  ...
  const { x, y } = canvasCoords(rt, event)
  rt.fullscreen?.rankedInput?.onLayout()
  rt.fullscreen?.rankedInput?.onDoubleClick(x, y)
  const cell = cellAt(rt, event)
  if (cell) {
    rt.callbacks.onChord(cell.row, cell.col)
  }
...
function updateUiHover(rt: GameCanvasRuntime, x: number, y: number): void {
  ...
  if (rt.fullscreen.isLogOpen?.() || rt.fullscreen.isLeaderboardOpen?.()) {
...
export function onMouseUp(rt: GameCanvasRuntime, event: MouseEvent): void {
  ...
  if (event.button === 0 || event.button === 2) {
    const { x, y } = canvasCoords(rt, event)
    rt.fullscreen?.rankedInput?.onUp(event.button === 2 ? 2 : 0, x, y)
  }
  if (event.button === 0) {
    const { x, y } = canvasCoords(rt, event)
...
export function onPointerDown(rt: GameCanvasRuntime, event: PointerEvent): void {
  ...
  const { x, y } = canvasCoords(rt, event)
  rt.fullscreen?.rankedInput?.onLayout()
  rt.fullscreen?.rankedInput?.onDown(0, x, y, event.buttons)
...
export function onPointerUp(rt: GameCanvasRuntime, event: PointerEvent): void {
  if (!useTouchPointerInput(rt.state.width)) return
  const { x, y } = canvasCoords(rt, event)
  rt.fullscreen?.rankedInput?.onUp(0, x, y)
