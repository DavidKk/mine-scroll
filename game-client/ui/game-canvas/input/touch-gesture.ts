...
export function sameCell(a: { row: number; col: number }, b: { row: number; col: number }): boolean {
  return a.row === b.row && a.col === b.col
}

export function classifyPointerEnd(dx: number, dy: number, mode: 'idle' | 'swipe-flag', thresholds: GestureThresholds): PointerEndClassification {
...
export function detectSwipeDuringMove(dy: number, mode: 'idle' | 'swipe-flag', thresholds: GestureThresholds): 'idle' | 'swipe-flag' {
  if (mode === 'swipe-flag') return 'swipe-flag'
  if (Math.abs(dy) > thresholds.swipeThresholdY) return 'swipe-flag'
  return 'idle'
}

export function isDoubleTap(cell: { row: number; col: number }, now: number, state: DoubleTapState): boolean {
