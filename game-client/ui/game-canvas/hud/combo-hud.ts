...
import { COMBO_HUD_TIER_THRESHOLDS, comboHudGlowRgba, drawComboRailInteriorArcs, getComboFeedbackPalette, getComboHudTier, getComboRailFilter } from '../../hud-feedback-fx.ts'
...
function drawComboHudDigitArcs(
  ...
  for (let i = 0; i < boltCount; i += 1) {
    ...
    drawElectricBolt(shellCtx, edgeX, edgeY, tipX, tipY, palette, fade * (0.55 + seededUnit(seed + 2) * 0.45), scale, seed, 3, i % 2 === 0)
  }
  shellCtx.restore()
}
...
function drawComboHudLabel(shellCtx: CanvasRenderingContext2D, cx: number, y: number, label: string, palette: ComboFeedbackPalette, scale: number, displayCombo: number): void {
  const tier = getComboHudTier(displayCombo)
  const isOverload = tier >= 3
  const now = Date.now()
...
export function drawComboRailGlow(
...
export function drawComboHud(rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, cx: number, y: number, combo: number, scale: number): void {
  if (combo <= 1) return
  ...
  const overloadProgress = getComboTier3Progress(displayCombo)
  const glowAlpha = 0.28 + Math.min(0.3, displayCombo * 0.012) + pulse * 0.1 + (tier >= 3 ? 0.12 + overloadProgress * 0.16 : 0)
  ...
  const underlineW = Math.min(tier >= 3 ? 120 * scale : 96 * scale, (tier >= 3 ? 40 : 32) * scale + String(displayCombo).length * (tier >= 3 ? 17 : 15) * scale)
