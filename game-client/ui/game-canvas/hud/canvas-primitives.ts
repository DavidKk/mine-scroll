...
import { type GameUiPanelName, getGameUiPanel } from '../../game-assets.ts'
...
export function drawArcadeGlow(_rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, color: string, intensity = 1): void {
  shellCtx.save()
...
export function drawRuntimePanelV3Fx(
  ...
  const glow = shellCtx.createRadialGradient(x + w / 2, y + h / 2, w * 0.05, x + w / 2, y + h / 2, w * 0.62)
  ...
  for (let i = 0; i < 8; i += 1) {
    ...
    const sparkX = side === 0 ? x + w * local : side === 1 ? x + w : side === 2 ? x + w * (1 - local) : x
    const sparkY = side === 0 ? y : side === 1 ? y + h * local : side === 2 ? y + h : y + h * (1 - local)
  ...
  if (actionProgress > 0) {
    ...
    const burst = shellCtx.createRadialGradient(x + w / 2, centerY, h * (0.08 + t * 0.1), x + w / 2, centerY, h * (0.32 + t * 0.62))
...
export function drawUiPanelImageBounds(
...
export function drawUiPanelImage(rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, name: GameUiPanelName, x: number, y: number, w: number, h: number, scale = 1): boolean {
  return drawUiPanelImageBounds(rt, shellCtx, name, x, y, w, h, scale) !== null
}

export function drawFeedbackAsset(
  _rt: GameCanvasRuntime,
  shellCtx: CanvasRenderingContext2D,
...
export function drawFilteredFeedbackAsset(
  _rt: GameCanvasRuntime,
  shellCtx: CanvasRenderingContext2D,
  image: HTMLImageElement,
  cx: number,
...
export function drawSheetFrameContained(
  _rt: GameCanvasRuntime,
  shellCtx: CanvasRenderingContext2D,
  image: HTMLImageElement,
  frameIndex: number,
...
export function setFittedMonoFont(
  _rt: GameCanvasRuntime,
  shellCtx: CanvasRenderingContext2D,
  text: string,
  maxWidth: number,
...
export function drawTopHudChip(
  rt: GameCanvasRuntime,
  shellCtx: CanvasRenderingContext2D,
  x: number,
  y: number,
...
export function drawBottomEnergyRail(rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, pressure: ScrollPressureState | undefined, shellW: number, _shellH: number): void {
