...
export function isScoreCountUpAnimating(rt: GameCanvasRuntime, now: number): boolean {
  return rt.state.scoreCountUpStartedAt > 0 && now - rt.state.scoreCountUpStartedAt < RUNTIME_CONSTANTS.SCORE_COUNT_UP_MS && rt.state.scoreCountUpTo > rt.state.scoreCountUpFrom
}
...
export function resolveScoreCountUpDisplay(rt: GameCanvasRuntime, now: number, actualScore: number): { score: number; animating: boolean } {
...
export function beginScoreCountUp(rt: GameCanvasRuntime, scoreAfter: number, scoreAdded: number, now: number): void {
  ...
  const fromScore = Math.min(replacing ? currentScoreHudValue(rt, now, rt.state.scoreCountUpTo) : (rt.state.lastDisplayedScore ?? scoreBefore), scoreAfter)
...
function drawScoreDigits(_rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, text: string, x: number, cy: number, maxW: number, maxH: number): boolean {
...
export function drawScoreHud(rt: GameCanvasRuntime, shellCtx: CanvasRenderingContext2D, x: number, y: number, score: number, scale: number): void {
  ...
  shellCtx.save()
  shellCtx.beginPath()
  shellCtx.rect(digitX - clipPadX, digitCy - digitMaxH / 2 - clipPadY, digitMaxW + clipPadX * 2, digitMaxH + clipPadY * 2)
  shellCtx.clip()
