...
export const THEME = {
  ...
  numbers: ['', '#60a5fa', '#4ade80', '#f472b6', '#fbbf24', '#c084fc', '#22d3ee', '#ec4899', '#fb923c'] as const,
} as const

/** Shrink cells on large boards to keep canvas in viewport; fixedCellSize forces a fixed width. */
export function computeGridMetrics(rows: number, cols: number, maxGrid?: { width: number; height: number }, fixedCellSize?: number): GridMetrics {
...
export function getCanvasSize(rows: number, cols: number, grid: GridMetrics): { width: number; height: number } {
...
export function cellPixelOrigin(row: number, col: number, gridOriginX: number, gridOriginY: number, grid: GridMetrics): { x: number; y: number } {
  return {
...
export function computeViewportCellSize(
  ...
  const aestheticMax = viewportW < 520 ? 23 : viewportW < 760 ? 25 : viewportW < 1100 ? 28 : 30
