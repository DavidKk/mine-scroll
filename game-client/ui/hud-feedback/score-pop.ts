...
export function resolveStripWrapBounds(cx: number, cy: number, maxW: number, maxH: number, image: HTMLImageElement | null, scale = 1): ScorePopStripOrbit {
...
export function drawScorePopBottomStrip(ctx: CanvasRenderingContext2D, opts: ScorePopBottomStripDrawOptions): void {
  ...
  drawFilteredContainedFeedbackAsset(ctx, scoreStrip, stripCx, stripCy, chipW, stripDrawH, getComboRailFilter(comboTier), 1, progress.alpha)
...
export function drawScorePopFx(ctx: CanvasRenderingContext2D, opts: ScorePopFxDrawOptions): void {
  const { cx, cy, scoreText, comboTier, progress, stageScale, scorePopBase, assetMaxW, assetMaxH, fontFamily, drawFallbackFx } = opts
  ...
  const drawn = drawFilteredContainedFeedbackAsset(ctx, scorePopBase, 0, assetCyOffset, assetMaxW, assetMaxH, tierFilter, 0.92, assetAlpha)
...
export function resolveScorePopV3PreviewLayout(canvasW: number, canvasH: number, progress: ScorePopFxProgress): ScorePopV3Layout {
