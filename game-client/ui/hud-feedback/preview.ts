...
export function drawComboBurstPreviewDecorations(
  ...
  for (let i = 0; i < boltCount; i += 1) {
    ...
    drawElectricBolt(ctx, x1, y1, x2, y2, palette, lifeAlpha, Math.max(1, effectRadius * 0.0065), seed + 18.6, getComboHudTier(combo) >= 2 ? 4 : 3, i % 2 === 0)
  }
  ctx.restore()
}

export function drawScorePopPreviewDecorations(ctx: CanvasRenderingContext2D, cx: number, cy: number, w: number, h: number, progress: ScorePopFxProgress, comboTier = 0): void {
