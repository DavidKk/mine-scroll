...
export function drawSimpleFlagMark(ctx: CanvasRenderingContext2D, x: number, y: number, g: GridMetrics): void {
...
export function drawWavingFlagMark(ctx: CanvasRenderingContext2D, x: number, y: number, g: GridMetrics, nowMs: number): void {
  const flag = getGameCutout('flag-blue')
  const sprites = getTileSprites()
  const img = flag ?? sprites?.flag
  if (!img) return
  ...
  drawFxSpriteFrame(ctx, 'flag-wave', progress, cx, cy, g.cellSize * tuning.spriteW, g.cellSize * tuning.spriteH, tuning.spriteAlpha * 0.55, { loop: true })
  ...
  if (flag) {
    ...
    ctx.translate(cx, cy)
    ctx.rotate(sway)
    drawImageContained(ctx, flag, -g.cellSize / 2, -g.cellSize / 2, g.cellSize, g.cellSize, GAME_ASSET_TUNING.cutouts.flagScale * 1.08)
    ctx.restore()
  } else {
    drawImageContained(ctx, img, x, y, g.cellSize, g.cellSize, GAME_ASSET_TUNING.cutouts.flagScale)
  }
