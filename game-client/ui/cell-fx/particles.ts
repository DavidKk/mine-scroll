...
function drawSoftFireflyDot(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, color: string, alpha: number, glow = true): void {
  if (glow) {
...
export function drawFeedbackStripLightWrap(
  ...
  const sampleCoil = (u: number, slide: number, bulge: number): CoilPoint => {
    ...
    const angle = u * coil * Math.PI * 2 + slide + Math.sin(u * 5.4 + bulge) * 0.48 + Math.cos(u * 3.2 + slide * 0.42) * 0.26
    const wobbleY = Math.sin(u * 12.8 + slide * 1.2 + bulge) * hh * 0.12 + Math.sin(u * 6.7 + bulge * 0.8) * hh * 0.07
  ...
  const addFly = (u: number, slide: number, bulge: number, color: string, seedI: number, trail: number, bright: number, jitterX: number, jitterY: number): void => {
    const uClamped = Math.max(0, Math.min(1, u))
    const pt = sampleCoil(uClamped, slide - trail * 0.12, bulge)
    if (pt.tip <= 0) return
    const twinkleRate = 0.7 + hash01(seedI + 11) * 1.6
    const twinkle = bright * (0.28 + Math.sin(phase * Math.PI * 2 * twinkleRate + seedI * 2.17 + trail * 0.9) * 0.72)
  ...
  for (let i = 0; i < flyCount; i += 1) {
    ...
    const jitterX = (hash01(seedI + 8) - 0.5) * hw * 0.05 + Math.sin(phase * Math.PI * 2 * (0.4 + hash01(seedI + 9)) + seedI) * hw * 0.028
    const jitterY = (hash01(seedI + 10) - 0.5) * hh * 0.14 + Math.sin(phase * Math.PI * 2 * (0.55 + hash01(seedI + 12)) + seedI * 1.4) * hh * 0.09
    ...
    for (let trail = 0; trail <= trailLen; trail += 1) {
      ...
      addFly(u - trail * trailStep * dir, slide, bulge, color, seedI, trail, trail === 0 ? 0.85 + hash01(seedI + 17) * 0.15 : 0.35 + hash01(seedI + 18) * 0.25, jitterX, jitterY)
    }
  }
