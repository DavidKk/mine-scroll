...
import { drawImageContained, type GameCutoutName, getGameCutout } from '../game-assets.ts'
...
export function drawBoardCellOverlays(
  ...
  const isPointer = options.pointer && options.pointer.row === view.row && options.pointer.col === view.col
  ...
  if (view.revealed && !view.isMine && (view.adjacentMines ?? 0) > 0 && isPointer) {
    drawDigitAmbientOverlay(ctx, x, y, g, view.adjacentMines ?? 0, options.nowMs)
  }
}

function drawCellFlagSwipeTargetOverlay(ctx: CanvasRenderingContext2D, x: number, y: number, g: GridMetrics, nowMs: number): void {
...
export function drawAiHintCutout(ctx: CanvasRenderingContext2D, x: number, y: number, g: GridMetrics, hint: AiHintDisplay): void {
  const cx = x + g.cellSize / 2
  const cy = y + g.cellSize / 2
