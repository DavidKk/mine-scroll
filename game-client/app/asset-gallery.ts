import { backdropNavItems, type BackdropPanelId, mountBackdropPanels } from './asset-gallery/ambient-backdrop-lab.ts'
import { audioNavItems, type AudioPanelId, mountAudioPanels } from './asset-gallery/audio-lab.ts'
import { type EffectPanelId, mountEffectPanels } from './asset-gallery/cell-effects.ts'
import { type GameUiLabPanelId, gameUiNavItems, mountGameUiPanels } from './asset-gallery/game-ui-lab.ts'
...
export function mountAssetGallery(root: HTMLElement, section: AssetLabSection, initialPanelId: string | null, onNavigate: (path: string) => void): () => void {
  ...
  if (section === 'sprites') {
    ...
    if (sections.length === 0) {
      ...
      empty.className = 'admin-page__section admin-page__section-body'
  ...
  if (section === 'animations') {
    ...
    disposers.push(
      mountLabSection(
        ...
        (id) => effects.getPanel(id as EffectPanelId),
        initialPanelId,
        { onPanelHide: (id) => effects.hidePanel(id as EffectPanelId) }
      )
    )
    return () => {
      for (const dispose of disposers) dispose()
