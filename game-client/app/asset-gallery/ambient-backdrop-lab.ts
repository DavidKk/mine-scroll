...
import { BACKDROP_LAB_PRESETS, type BackdropLabPreset, type BackdropMood, drawAmbientBackdrop } from '../../ui/ambient-backdrop.ts'
...
function paintBackdropFrame(ctx: CanvasRenderingContext2D, w: number, h: number, nowMs: number, mood: BackdropMood, status: GameStatus): void {
...
function createBackdropPreview(preset: BackdropLabPreset, getFps: () => number, width: number, height: number, className: string): LivePreview | null {
...
function createSinglePresetPanel(preset: BackdropLabPreset): { panel: HTMLElement; dispose: () => void } {
  ...
  const preview = createBackdropPreview(preset, getFps, PREVIEW_W, PREVIEW_H, 'asset-lab__preview-canvas asset-lab__preview-canvas--backdrop')
  if (preview) {
    previewWrap.append(preview.canvas)
    previewWrap.append(createPreviewExpandButton(() => itemFromCanvas(preview.canvas, preset.label, preset.description)))
  }
...
function createComparePanel(): { panel: HTMLElement; dispose: () => void } {
  ...
  for (const tier of tierPresets) {
    ...
    const preview = createBackdropPreview(tier, getFps, TIER_PREVIEW_W, TIER_PREVIEW_H, 'asset-lab__preview-canvas asset-lab__preview-canvas--backdrop-tier')
    if (preview) {
      previewWrap.append(preview.canvas)
      previewWrap.append(createPreviewExpandButton(() => itemFromCanvas(preview.canvas, tier.label, tier.description)))
