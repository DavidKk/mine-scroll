...
export function createMineLiveCanvas(sprites: TileSprites): LivePreview | null {
  ...
  const stopLoop = startPreviewLoop(canvas, () => {
    ...
    const animating = explosionStart !== null || settled
    if (!animating && now - lastFrameAt < 1000 / 12) return
    lastFrameAt = now
...
export function createAnimPreview(id: EffectPanelId, sprites: TileSprites, getFps: () => number, baseFps: number): LivePreview | null {
  if (id === 'cells') return createCellLiveCanvas(sprites, getFps, baseFps)
  if (id === 'board-interactions-v3') {
    return createLoopCanvas(
      (ctx, w, h, now) => {
