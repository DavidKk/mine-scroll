import { initPreviewCanvas, type LivePreview, measurePreviewCanvas, startPreviewLoop } from './cell-effect-scenes.ts'

export function scaledTime(now: number, fps: number, baseFps: number): number {
  return now * (fps / baseFps)
}

export function createLoopCanvas(draw: (ctx: CanvasRenderingContext2D, w: number, h: number, now: number) => void, getFps: () => number, baseFps: number): LivePreview | null {
